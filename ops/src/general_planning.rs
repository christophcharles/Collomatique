    ...
    >(
        ...
        match self {
            ...
            GeneralPlanningUpdateOp::UpdatePeriodWeekCount(period_id, week_count) => {
                ...
                // `week_count..old_week_count`). A non-empty colloscope cell on
                // any of them, or a pattern excluding any of them, must be
                // cleaned first (the former loses data, the latter would block
                // the elementary week removals).
                let removed_week_ids: Vec<collomatique_state_colloscopes::WeekId> = (*week_count
                    ...
                    .collect();
                let removed_week_set: std::collections::BTreeSet<
            ...
            GeneralPlanningUpdateOp::DeletePeriodAndWeeks(period_id) => {
                    ...
                    .iter()
                {
                    ...
                    if rule.excluded_periods().contains(period_id) {
                        ...
                        let new_rule = collomatique_state_colloscopes::pairings::PairingRule::new(
                            antecedent,
                            consequent,
                            excluded_periods,
                            soft,
                        ...
                        .expect(
                            "removing an excluded period preserves the distinct-subject invariant",
                        );
                        return Some(CleaningOp {
                            warning:
                                GeneralPlanningUpdateWarning::LoosePairingRuleExclusionForPeriod(
