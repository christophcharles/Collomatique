...
//! structurally ([crate::payload]), so an error variant added in `colloscopes/ops/` reaches a
...
/// added in `colloscopes/ops/` reaches a script with no change on this side.
...
pub(crate) fn refused(py: Python<'_>, error: &collomatique_ops::UpdateError) -> PyErr {
    ...
    match crate::payload::to_py(py, error) {
        ...
        // Nothing in `colloscopes/ops/` can fail to serialize — the error types are derived
