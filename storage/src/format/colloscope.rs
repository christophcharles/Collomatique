...
pub struct Colloscope {
    /// Keyed by `(slot_id, week)`, with a derived key set: the cells
...
mod tests {
    ...
    fn duplicate_cell_key_is_rejected() {
        let value = json!({
            "interrogations": [
                { "slot_id": 8, "week": 0, "assigned_groups": [0] },
                { "slot_id": 8, "week": 0, "assigned_groups": [1] }
            ],
            "group_lists": []
        });
    ...
    fn same_slot_on_two_weeks_is_accepted() {
        let value = json!({
            "interrogations": [
                { "slot_id": 8, "week": 0, "assigned_groups": [0] },
                { "slot_id": 8, "week": 1, "assigned_groups": [0] }
            ],
            "group_lists": []
        });
    ...
    fn duplicate_assigned_group_is_rejected() {
        let value = json!({ "slot_id": 8, "week": 0, "assigned_groups": [0, 0] });
    ...
    fn missing_field_is_rejected() {
        let value = json!({ "slot_id": 8, "week": 0 });
        assert!(serde_json::from_value::<Interrogation>(value).is_err());
    }
