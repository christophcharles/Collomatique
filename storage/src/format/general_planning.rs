...
mod tests {
    ...
    fn missing_optional_field_is_rejected() {
        ...
        assert!(serde_json::from_value::<Week>(json!({ "interrogations": true })).is_err());
    }
    ...
    fn unknown_field_is_rejected() {
        ...
        assert!(
            serde_json::from_value::<Week>(
                json!({ "interrogations": true, "annotation": null, "extra": 1 })
            )
            .is_err()
        );
    }
    ...
    fn empty_annotation_is_rejected() {
        assert!(
            serde_json::from_value::<Week>(json!({ "interrogations": true, "annotation": "" }))
                .is_err()
        );
    }
}
