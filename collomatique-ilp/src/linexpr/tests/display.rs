...
fn constraint_display_no_constant_equals() {
    let expr =
        2 * Expr::<String>::var("a") - 3 * Expr::<String>::var("b") + 4 * Expr::<String>::var("c");
    let constraint = expr.eq(&Expr::<String>::constant(0.0));
    assert_eq!(format!("{}", constraint), "2*a + (-3)*b + 4*c = 0");
}
...
fn constraint_display_with_constant_equals() {
    let expr = 2 * Expr::<String>::var("a") - 3 * Expr::<String>::var("b")
        + 4 * Expr::<String>::var("c")
        + 2;
    let constraint = expr.eq(&Expr::<String>::constant(1.0));
    assert_eq!(format!("{}", constraint), "2*a + (-3)*b + 4*c + 1 = 0");
}
