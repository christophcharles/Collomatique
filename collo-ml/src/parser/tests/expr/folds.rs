...
fn fold_accepts_simple_fold() {
    let cases = vec![
        "fold x in students with acc = 0 { acc + $Var(x) }",
...
fn fold_accepts_fold_with_where() {
    let cases = vec![
        "fold x in students with acc = 0 where x.is_active { acc + $Var(x) }",
...
fn fold_accepts_nested_folds() {
    let cases = vec![
        "fold x in x_list with acc1 = 0 { fold y in y_list with acc2 = 0 { acc2 + $Var(x, y) } }",
...
fn fold_accepts_fold_in_larger_expression() {
    let cases = vec![
        "(fold x in x_list with acc = 0 { acc + $V(x) }) + 10",
        "2 * (fold x in x_list with acc = 1 { acc * $V(x) })",
        "(fold x in x_list with acc = 0 { acc + $V(x) }) - (fold y in y_list with acc = 0 { acc + $V(y) })",
        "fold x in x_list with acc = 0 { acc + $V(x) + 5 }", // 5 inside fold body
        "fold x in x_list with acc = 0 { acc + $V(x) } + 5", // 5 outside fold
    ];
    for case in cases {
        let result = ColloMLParser::parse(Rule::expr_complete, case);
        assert!(result.is_ok(), "Should parse '{}': {:?}", case, result);
...
fn fold_accepts_fold_with_collection_operations() {
    let cases = vec![
        "fold s in students + teachers with acc = 0 { acc + $V(s) }",
...
fn rfold_accepts_simple_rfold() {
    let cases = vec![
        "rfold x in students with acc = 0 { acc + $Var(x) }",
...
fn rfold_accepts_rfold_with_where() {
    let cases = vec![
        "rfold x in students with acc = 0 where x.is_active { acc + $Var(x) }",
...
fn rfold_accepts_nested_rfolds() {
    let cases = vec![
        "rfold x in x_list with acc1 = 0 { rfold y in y_list with acc2 = 0 { acc2 + $Var(x, y) } }",
...
fn fold_rejects_incomplete_expressions() {
    let cases = vec![
        "fold x in x_list with acc = 0 {",         // missing closing brace
        "fold x in x_list with acc = 0 acc + x",   // missing braces
        "fold x in x_list with acc = 0",           // missing body
        "fold x in x_list acc = 0 { acc + x }",    // missing 'with'
        "fold x in x_list with { acc + x }",       // missing accumulator definition
        "fold x in x_list with acc { acc + x }",   // missing '=' in accumulator
        "fold x x_list with acc = 0 { acc + x }",  // missing 'in'
        "fold in x_list with acc = 0 { acc + x }", // missing iterator variable
    ];
    for case in cases {
        let result = ColloMLParser::parse(Rule::expr_complete, case);
...
fn rfold_rejects_incomplete_expressions() {
    let cases = vec![
        "rfold x in x_list with acc = 0 {",       // missing closing brace
        "rfold x in x_list with acc = 0 acc + x", // missing braces
        "rfold x in x_list with acc = 0",         // missing body
        "rfold x in x_list acc = 0 { acc + x }",  // missing 'with'
    ];
    for case in cases {
        let result = ColloMLParser::parse(Rule::expr_complete, case);
        assert!(result.is_err(), "Should reject '{}': {:?}", case, result);
...
fn fold_rejects_reserved_keywords_as_names() {
    let cases = vec![
        "fold let in x_list with acc = 0 { acc + let }", // 'let' is reserved
        "fold in in x_list with acc = 0 { acc + in }",   // 'in' is reserved
        "fold x in x_list with fold = 0 { fold + x }",   // 'fold' is reserved as accumulator
        "fold x in x_list with where = 0 { where + x }", // 'where' is reserved as accumulator
        "fold x in x_list with with = 0 { with + x }",   // 'with' is reserved as accumulator
    ];
    for case in cases {
        let result = ColloMLParser::parse(Rule::expr_complete, case);
        assert!(result.is_err(), "Should reject '{}': {:?}", case, result);
