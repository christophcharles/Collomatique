...
impl TypeName {
    pub(super) fn from_pest(pair: Pair<Rule>) -> Result<Self, AstError> {
        let span = Span::from_pest(&pair);
...
impl SimpleTypeName {
    ...
    pub(super) fn from_struct_type(pair: Pair<Rule>) -> Result<Self, AstError> {
...
impl Expr {
    ...
    fn from_list_comprehension(pair: Pair<Rule>) -> Result<Self, AstError> {
        ...
        let vars_and_collections = vars.into_iter().zip(collections).collect();
