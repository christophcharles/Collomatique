...
| 14 | Salle | string or empty | Room preference(s) for the interrogation. If empty, no preference. Multiple preferences can be separated by semicolons (e.g. `A101;!B302`). Each preference is parsed independently: a plain room name (e.g. `A101`) is a suggestion, a name prefixed with `!` (e.g. `!A101`) is a demand. A name suffixed with `+` (e.g. `A101+` or `!A101+`) enables prep sharing: when the solver assigns this interrogation to the named room, prep students from other requests may share the room (capacity permitting). If the same room appears multiple times with different flags, they are merged: demand wins over suggestion, sharing wins over no sharing (a warning is emitted). Unregistered room names are allowed but will trigger a warning: closest-room resolution will not be available in case of double occupancy. To avoid warnings, list the room in the rooms CSV with Priorité = -1. |
