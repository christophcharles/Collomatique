...
fn new_rejects_zero_duration() {
    let duration = NonZeroMinutes::new(0);
...
fn new_accepts_one_minute_duration() {
    let duration = NonZeroMinutes::new(1);

    assert_eq!(duration, Some(NonZeroMinutes(NonZeroU32::new(1).unwrap())));
}

#[test]
fn new_accepts_typical_duration() {
    let duration = NonZeroMinutes::new(90);
...
fn new_accepts_max_duration() {
    let duration = NonZeroMinutes::new(u32::MAX);

    assert_eq!(
        duration,
        Some(NonZeroMinutes(NonZeroU32::new(u32::MAX).unwrap()))
    );
}
...
fn from_non_zero_u32() {
    ...
    let duration: NonZeroMinutes = non_zero.into();
...
fn time_delta_for_large_duration() {
    let duration = NonZeroMinutes::new(1440).unwrap(); // 24 hours
    let time_delta = duration.time_delta();
...
fn display_shows_single_digit_minutes() {
    let duration = NonZeroMinutes::new(65).unwrap();
...
fn display_for_multiple_hours() {
    let duration = NonZeroMinutes::new(185).unwrap(); // 3h05
...
fn display_for_exactly_multiple_hours() {
    let duration = NonZeroMinutes::new(180).unwrap(); // 3h00
...
fn display_for_full_day() {
    let duration = NonZeroMinutes::new(1440).unwrap(); // 24h00
...
fn ordering_less_than() {
    let duration1 = NonZeroMinutes::new(30).unwrap();
    let duration2 = NonZeroMinutes::new(60).unwrap();
...
fn ordering_greater_than() {
    ...
    let duration2 = NonZeroMinutes::new(90).unwrap();
...
#[test]
fn equality() {
    let duration1 = NonZeroMinutes::new(75).unwrap();
    let duration2 = NonZeroMinutes::new(75).unwrap();
...
#[test]
fn inequality() {
    let duration1 = NonZeroMinutes::new(60).unwrap();
    let duration2 = NonZeroMinutes::new(90).unwrap();
