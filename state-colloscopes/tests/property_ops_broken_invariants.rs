//! Property tests over generated elementary-op sequences — new-checker mirror
//!
//! This is a deliberate *duplicate* of `property_ops.rs`: the same
//! deterministic, seed-driven op sequences, but the invariant oracle is
//! swapped from the old `InnerData::check_invariants` to the new
//! `InnerData::broken_invariants`. Because `broken_invariants` consumes no
//! RNG, every duplicated trajectory stays byte-identical to its original, so
//! this harness asserts that **every valid structure the property harness
//! visits comes out clean under the new checker** — i.e. reports
//! `Ok(BTreeSet::new())` (no logic errors, no dangling refs, no convergence
//! breaks).
//!
//! This covers only the forward direction (old-valid ⇒ new-valid). The reverse
//! regression (a state the old checker rejects but the new one accepts) is
//! deliberately not covered here and will come later. The old `property_ops.rs`
//! harness is kept and runs independently.
//!
...
/// The swapped oracle: the new whole-model checker must report a fully clean
/// document (no logic errors, no dangling references, no convergence breaks)
/// for every state the old harness considers valid.
...
fn invariants_hold_and_errors_are_atomic() {
    harness::for_each_seed(
        "broken_invariants::invariants_hold_and_errors_are_atomic",
        &CONFIG,
        |rng, log, stats| {
...
fn undo_all_and_redo_all_round_trip() {
    harness::for_each_seed(
        "broken_invariants::undo_all_and_redo_all_round_trip",
        &CONFIG,
        |rng, log, stats| {
...
fn random_undo_redo_apply_walk() {
    harness::for_each_seed(
        "broken_invariants::random_undo_redo_apply_walk",
        &CONFIG,
        |rng, log, stats| {
            let (mut state, mut snapshots) = harness::bootstrap(rng);
            assert_clean(state.get_data().get_inner_data());
...
fn apply_then_apply_rev_is_identity() {
    ...
    harness::for_each_seed(
        "broken_invariants::apply_then_apply_rev_is_identity",
        &CONFIG,
        |rng, log, stats| {
...
fn sessions_commit_and_cancel_randomly() {
    harness::for_each_seed(
        "broken_invariants::sessions_commit_and_cancel_randomly",
        &CONFIG,
        |rng, log, stats| {
            let (mut state, _) = harness::bootstrap(rng);
            assert_clean(state.get_data().get_inner_data());
